package EENG4990.Final_Project.EmergencyComm;
...
public class BackService extends Service implements RecognitionListener, SensorEventListener {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        runnable = new Runnable() {
            public void run() {
//               Instantiate Speech Recognizer - handles the voice detection and language parsing
...
//               Assign RecognitionListener to the SpeechRecognizer - event handler for recognizer events
                speech.setRecognitionListener(BackService.this);
//               Set options for SpeechRecognizer
                ...
                if (start_handler) {
//                   We use audio manager to mute all device sounds when app is running
...
//                  Start the listening / voice detection, using recognizerIntent as options
                    speech.startListening(recognizerIntent);

//                  restart the runnable every 5 seconds
                    handler.postDelayed(runnable, 5000);
                }
            }
        };

//      add 1 second delay as a buffer
        handler.postDelayed(runnable, 1000);

//       if service ended in mid cycle then partial state saved and reused on new service start
        return START_STICKY;
    }
    ...
    public void onResults(Bundle results) {
        try {
            ...
            String text = "";
//          If results exist get the value from the recognized text
            if (matches != null) {
...
//          Check for user command "call to"
//          If yes, get the phone Number of the person to call
//          Then, start a TELEPHONE intent, signalled by the URI `tel:(number)`
...
//              #fire off TELEPHONE CALL TO itnetn
